...
@Time     :   2022-11-03 11:53
@Project  :   PyTorchBasic-train_lenet_augmentation
'''
...
train_transform = transforms.Compose([
    ...
    # 数据增强，灰度处理
    transforms.RandomGrayscale(0.95),
    transforms.ToTensor(),
...
# 推理
test_path = '../../data/RMB_split/custom'
...
for iteration, data in enumerate(test_loader):
    ...
    face_value = 1 if preds[0].item() == 0 else 100
    ...
    plt.imshow(img)
    plt.title(f'LeNet got {face_value} Yuan')
    plt.show()
    plt.pause(0.5)
    plt.close()

avg_test_loss = test_loss / len(test_loader)
print(f'\nTest: Loss: {avg_test_loss:.4f} Acc: {test_correct / test_total:.2%}')
